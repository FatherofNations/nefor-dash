"use client";
/* ═══ Тело пасхалки «Защита счёта» ═══
   Живёт отдельным модулем и подключается ТОЛЬКО динамическим import() из
   useTowers — вместе со стилями и всем движком td/*. Обычный посетитель
   дашборда этот код не качает. */
import "@/styles/td.css";
import { bake } from "./pixel";
import { ENEMY_ART, EnemyKey, NEON, TOWER_ART, TowerKey } from "./art";
import { ENEMIES, ENEMY_ORDER, FALLBACK_ACCOUNT, TOWERS, statChips } from "./defs";
import { CORE_SEL, buildField } from "./field";
import { Game, Hooks, Stats, TowerInfo } from "./game";
...
function accountBinding() {
  /* Узел ищем заново при каждом обращении: контент дашборда живёт в
     dangerouslySetInnerHTML и может переехать (своп роутов, смена вида
     сайдбара). Ссылка, взятая один раз, после такого указывала бы на
     отсоединённый узел, и восстановление баланса тихо не сработало бы. */
  const node = () => document.querySelector<HTMLElement>(CORE_SEL);
  const el = node();
...
/** Всё поведение игры: возвращает функцию полной уборки. */
export function mount(): () => void {
  // горячие клавиши идут в порядке колоды (от дешёвой к дорогой)
  const HOT: TowerKey[] = TOWERS.map((t) => t.key);
  /* Неон врага для подписей — тот же цвет «глаза», что и у спрайта
  ...
  const ENEMY_NEON: Record<EnemyKey, string> = {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      /* Escape отдаём панели tools, если она открыта: её обработчик живёт
         только на время открытия, и перехватывать его игре незачем. */
      if (e.code === "Escape" && !document.body.classList.contains("twk-open")) {
        game?.select(null);
        game?.closePanel();
      }
    ...
    const watchdog = window.setInterval(() => {
      ...
      if (document.hidden) return; // в фоне мерить нечего и незачем
      const now = anchorOf();
      if (!now) return;
      if (!anchor) {
        anchor = now;

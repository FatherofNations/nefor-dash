...
/* Каждые десять волн враги крепчают на 10% ОТ БАЗОВЫХ значений: HP, скорость,
   броня и сумма, которую уносит прорыв. Прибавка линейная, а не нарастающим
   итогом: 1 + 0.1 × десяток, то есть волны 1–9 → ×1.0, 10–19 → ×1.1,
   20–29 → ×1.2 … 90–100 → ×2.0. У Пени это ровно +2,2 HP и +1,8к урона по
   счёту за каждый десяток. Награду множитель не трогает — иначе рост
   сложности сам себя и оплачивал бы. */
export const decadeMul = (wave: number) => 1 + 0.1 * Math.floor(wave / 10);

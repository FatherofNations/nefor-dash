...
export type EasterTool = "pickaxe" | "rpg" | "td" | "tanks";
...
/* Ключ к тому, как выглядит референс: дым заливает ВЕСЬ канвас, а фигура
   занимает лишь его часть — свечение кромки и «размытый фон за стеклом»
   живут в зазоре между фигурой и краем канваса. Раньше фигура у нас была
   ровно по карточке, зазора не оставалось, и от эффекта была видна только
...
/* Канвас делаем ШИРЕ плашки, а лишнее прячем под её клип. Смысл в масштабе
   узора: дым живёт в координатах канваса, и в боксе 342×113 он выглядит мельче
   и резче, чем в демо-кадре 905×510. Увеличенный канвас показывает в карточке
   более крупный (а значит мягкий, без чётких линий) кусок того же дыма.
   Масштаб фигуры пересчитываем, чтобы она осталась ровно на своём месте:
   fit:contain вписывает картинку по ширине, 0.95 — поля препроцессора. */
const SMOKE_PAD = 60;
const SMOKE_SCALE = SLAB_W / ((SLAB_W + 2 * SMOKE_PAD) * 0.95);
...
export function useMiner(session: boolean, tool: EasterTool | null, onMined: () => void) {
  ...
  useEffect(() => {
    ...
    if (!session || !tool || tool === "td" || tool === "tanks") return;
    ...
    const smokeUniforms = (clr: string) => {
      ...
      return {
        ...
        // фон канваса чёрный, как сама плашка: канвас лежит внутри её клипа
        ...
        u_outerDistortion: 0.32, // мягче: меньше турбулентности, меньше чётких линий
        u_outerGlow: 1,
        u_innerGlow: 1,
        u_offset: -1,
        u_angle: 164,
